 ...
 * @param {any} response
 */
async function formatChart(response) {
  var data = response.data;
  var container = document.getElementById(response.chartType);
  switch (response.chartType) {
    case 'candlestick': {
      var chart = new Chart(container, {
        ...
        data: {
          datasets: [
            {
              label: response.pair.toUpperCase(),
              data: data,
              order: 0,
            },
            {
              ...
              type: 'line',
              data: response.purchases.map((entry) => ({ x: entry[0], y: entry[1] })),
    ...
    case 'traderBot': {
      let pair = response.pair.toUpperCase().split('/');
      new Chart(document.getElementById(response.chartType), {
        type: 'line',
        data: {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: `TraderBot cumulative PnL per ${response.groupBy} (${response.pair})`,
              font: { size: 14 },
            },
            tooltip: {
              callbacks: {
